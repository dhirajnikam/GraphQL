...
function makeAugmentedSchema(typeDefs, { enableRegex, skipValidateTypeDefs } = {}) {
    ...
    nodes.forEach((node) => {
        ...
        ["Create", "Update"].map((operation) => composer.createObjectTC({
            name: `${operation}${(0, upper_first_1.upperFirst)(node.plural)}MutationResponse`,
            fields: {
                info: `${operation}Info!`,
        ...
        if (!((_a = node.exclude) === null || _a === void 0 ? void 0 : _a.operations.includes("read"))) {
            composer.Query.addFields({
                [node.plural]: (0, resolvers_1.findResolver)({ node }),
            });
            composer.Query.addFields({
                [`${node.plural}Aggregate`]: (0, resolvers_1.aggregateResolver)({ node }),
            });
        }
        if (!((_b = node.exclude) === null || _b === void 0 ? void 0 : _b.operations.includes("create"))) {
            composer.Mutation.addFields({
                [`create${(0, upper_first_1.upperFirst)(node.plural)}`]: (0, resolvers_1.createResolver)({ node }),
            });
        }
        if (!((_c = node.exclude) === null || _c === void 0 ? void 0 : _c.operations.includes("delete"))) {
            composer.Mutation.addFields({
                [`delete${(0, upper_first_1.upperFirst)(node.plural)}`]: (0, resolvers_1.deleteResolver)({ node }),
            });
        }
        if (!((_d = node.exclude) === null || _d === void 0 ? void 0 : _d.operations.includes("update"))) {
            composer.Mutation.addFields({
                [`update${(0, upper_first_1.upperFirst)(node.plural)}`]: (0, resolvers_1.updateResolver)({
                    node,
                    schemaComposer: composer,
                }),
            });
